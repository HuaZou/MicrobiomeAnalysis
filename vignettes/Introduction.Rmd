...
+ Normalizing the OTU_table in phyloseq-class object

```{r}
ps_genus_transform_norm <- normalize(
    object = ps_genus_transform,
    method = "TSS")

head(ps_genus_transform_norm@otu_table@.Data, 3)
```
...
```{r}
ps_genus_transform_norm_trim <- trim_prevalence(
    object = ps_genus_transform,
    level = NULL,
    cutoff = 0.1,
    trim = "feature")

head(ps_genus_transform_norm_trim@otu_table@.Data, 3)
```
...
```{r}
ps_genus_transform_norm_trim_filter <- filter_abundance(
    object = ps_genus_transform_norm_trim,
    level = NULL,
    cutoff_mean = 1e-04,
    cutoff_one = 1e-03,
    unclass = TRUE)

head(ps_genus_transform_norm_trim_filter@otu_table@.Data, 3)
```
